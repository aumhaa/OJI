# name=OJI AKAI FL Studio Fire
# url=https://forum.image-line.com/viewtopic.php?p=1496543#p1496543
# receiveFrom=OJI AKAI FL Studio Fire
...
FLFireDeviceName = 'OJI Akai FL Studio Fire'
PadsW = 16
PadsH = 4
PadsStride = 16
...
class TFire():
	...
	def OnMidiMsg(self, event):
		...
		def HandleHeldPadsKeyInput(Data2):

			ParamDefVal = DotNote_Default
			ParamMax = 127
			...
			for m in range(0, len(self.HeldPads)):
				...
				if n >= channels.channelCount():
					return
				# bitPos = self.HeldPads[m] % PadsStride
				self.SetStepParam(self.HeldPads[m], pPitch, Data2)
				# SetStep(self.HeldPads[m], self.HeldPadsChanged)
				self.HeldPadsChanged = True
				if m == 0:
					self.DisplayTimedText('SetPitch: ' + utils.GetNoteName(Data2))
				channels.updateGraphEditor()
		...
		# debug(device.getName(), device.getName() == 'Sensel Morph')
		# if device.getName() == 'Sensel Morph':
		if device.getPortNumber() == 255:
			if event.midiId == 144:
				# debug('here', midi.MIDI_NOTEON)
				# we need to send to all instances of the fire script
				for index in range(device.dispatchReceiverCount()):
					device.dispatch(index, 159 + (event.data1 << 8) + (event.data2 << 16) )
				# debug('event dispatched:', event.midiId)
			event.handled = True
		if event.midiChan == 15:
			debug('event data:', event.data1, event.data2)
			self._last_received_note = event.data1
			debug('_last_recieved_note:', self._last_received_note)
			HandleHeldPadsKeyInput(event.data1)
			event.handled = True
...
def OnInit():
	debug('isAssigned', device.isAssigned())
	debug('getPortNumber', device.getPortNumber())
	debug('getName', device.getName())
	debug('count:', device.dispatchReceiverCount())
	debug('info:', device.dispatchGetReceiverPortNumber(0))
	for index in range(device.dispatchReceiverCount()):
		debug('info for device:', index, 'port:', device.dispatchGetReceiverPortNumber(index))
	Fire.OnInit()
