...
const sqlite3 = require('../node_dependencies/node_modules/sqlite3').verbose();
// const sqlw = require('../node_dependencies/node_modules/sqlite3-wrapper');
...
// 		db.run('INSERT INTO filenames(filename, tags, shortname) VALUES(?, ?, ?) ON CONFLICT(filename) DO UPDATE SET tags=tags', [entry, value[entry].tags, value[entry].shortname], (err) => {
// 			if(err){
...
class PresetTagger {
	...
	// setDict = async(id, value) => {
	// 	for(var entry in value){
	// 		this.db.run('INSERT INTO filenames(filename, tags, shortname) VALUES(?, ?, ?) ON CONFLICT(filename) DO UPDATE SET tags=tags', [entry, value[entry].tags, value[entry].shortname], (err) => {
	// 			if(err){
	// 				debug('error:', err.message);
	// 			}
	// 		});
	...
	init_db = async () => {
		let tableCreation = 'CREATE TABLE IF NOT EXISTS filenames (filename TEXT PRIMARY KEY NOT NULL, tags TEXT NULL, shortname TEXT NOT NULL)'

		let db = await new sqlite3.Database('./testDB', (err) => {
				if(err) {
					debug('error opening sqlite db:', err.message);
				}
				else{
					debug('no problem with db open');
				}
		});
		let res = await db.run(tableCreation, (err) => {
			if(err){
				debug('error setting table:', err.message);
			}
			else{
				debug('no problem with table create');
			}
		});
		// let tableCreation = 'CREATE TABLE IF NOT EXISTS filenames (filename TEXT PRIMARY KEY NOT NULL, tags TEXT NULL, shortname TEXT NOT NULL)'
		return db;
	...
	init_from_js = async () => {
		...
		// temp comment: this.db = await this.init_db();
		debug('returned from creating db...');
	...
	scan_library_internal = async() => {
		...
		if( (fs.existsSync(this.library_dir)) && (fs.lstatSync(this.library_dir).isDirectory()) ){
			...
			try{
				...
				debug('setting dict...');
				await this.setDict(this.libraryDictId, this.library_data);
				return true
			}
			catch(e){
				return Promise.reject(e)
		...
		for(const filename of directoryFiles){
			...
			if( (fs.existsSync(file_path)) && (fs.lstatSync(file_path).isDirectory()) ){
				try{
					...
					debug('scan folder failed on:', file_path);
					return e;
				}
			}
				...
				try{
					...
					debug('scan file failed on:', file_path, filename);
					return e;
				}
			}
		}
